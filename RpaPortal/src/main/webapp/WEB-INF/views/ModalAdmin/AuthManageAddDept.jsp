...
<script type="text/javascript">
	...
	$(document).ready(function (e) {		
		ListAuthManageSearchDept(menuId, "", "");
	});
	
	// 직급 조회
	function ListAuthManageSearchDept(pMenuId, pMenuType, pSearchTxt) {
		$.ajax({
			url: "/AjaxAdmin/ListAuthManageSearchDept.do",
			type: "POST",
	...
	function makeGrid(pListDatas) {
	    ...
	    initdata.Cols = [                        
	        { Header: "부서코드", Type: "Text", Width: 170, SaveName: "deptCd", Align: "Center" },
	        { Header: "부서명", Type: "Text", Width: 170, SaveName: "deptNm", Align: "Center" }
	    ];
	
	    IBS_InitSheet(mySheet, initdata);
	    mySheet.SetEditable(0);
	    mySheet.SetEditableColorDiff(0);
	    mySheet.SetColFontUnderline("deptNm", true);
	    mySheet.SetDataLinkMouse("deptNm", true);
	...
	function mySheet_OnClick(Row, Col, Value, CellX, CellY, CellW, CellH) {
		...
		var userNm = mySheet.GetCellValue(Row, "deptNm");	
		var param = [];
		param.push(Row);		
		
		libraryFunc.createDialog("Confirm", null, null, null, null, "알림", userNm + " 부서 권한을 추가하시겠습니까?", null, CreateAuthManageConfirm, param);    
	}
	
	function CreateAuthManageConfirm(pOption, pParam){
		if (pOption.sdBtnKey == "o") {
			var authTypeCd = "RA001003";
			var authType = mySheet.GetCellValue(pParam[0], "deptCd");
	...
	$(document).on("click", "#btn_search", function (e) {
		...
		if(searchTxt == ""){
			libraryFunc.createDialog("Alert", null, null, null, null, "알림", "부서명을 입력하시기 바랍니다.", null, null, null);
    		return false;
		}
		
		ListAuthManageSearchDept(menuId, menuType, searchTxt);
	});
